...
class RegistroSaida(models.Model):
    ...
    #     LegalRep, on_delete=models.CASCADE, related_name="registro_saidas"
    # )
    legal_rep = models.ForeignKey(
        LegalRepStudent,
        on_delete=models.CASCADE,
        related_name="registro_saidas_legalrep",
        null=True,
        blank=True,
    )
    turma = models.ForeignKey(Turma, on_delete=models.CASCADE, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.student.name} - {self.date} - {self.time} - {self.legal_rep.legal_rep.name}"
